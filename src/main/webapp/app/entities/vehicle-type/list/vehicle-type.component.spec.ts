...
import { sampleWithRequiredData } from '../vehicle-type.test-samples';
...
describe('VehicleType Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.vehicleTypes?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to vehicleTypeService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getVehicleTypeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getVehicleTypeIdentifier).toHaveBeenCalledWith(entity);
